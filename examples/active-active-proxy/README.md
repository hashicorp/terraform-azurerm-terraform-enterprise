...
- Standard_D32a_v4 virtual machine type
- RHEL 7.9
...
- A valid CA certificate and private key for the MITM proxy that are both stored in the Azure Key Vault as secrets. These should be Base64 encoded versions of a PEM encoded certificate and private key.
...
3. SSH to bastion via: `$ ssh -N -p 22 -D localhost:5000 bastionuser@bastionhost -i ../path/to/id_rsa`
4. The TFE URL is now aacessible via bastion.
