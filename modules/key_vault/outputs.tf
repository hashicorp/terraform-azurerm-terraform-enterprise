output "tls_ca_cert" {
  value       = var.user_data_ca == null && var.load_balancer_type == "load_balancer" && var.load_balancer_public == false ? tls_self_signed_cert.ca[0].cert_pem : var.user_data_ca
  description = "Value to be provided for TFE ca_cert setting"
}

output "tls_cert" {
  value       = var.user_data_cert == null && var.load_balancer_type == "load_balancer" && var.load_balancer_public == false ? tls_locally_signed_cert.cert[0].cert_pem : var.user_data_cert
  description = "Value to be provided for Replicated TlsBootstrapCert setting"
}

output "tls_key" {
  value       = var.user_data_cert_key == null && var.load_balancer_type == "load_balancer" ? tls_private_key.cert[0].private_key_pem : var.user_data_cert_key
