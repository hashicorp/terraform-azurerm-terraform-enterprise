...
certificate=$(curl https://${key_vault_name}.vault.azure.net/secrets/${ca_cert_secret_name}?api-version=2016-10-01 -H "x-ms-version: 2017-11-09" -H "Authorization: Bearer $access_token" | jq -r .value)
private_key=$(curl https://${key_vault_name}.vault.azure.net/secrets/${ca_private_key_secret_name}?api-version=2016-10-01 -H "x-ms-version: 2017-11-09" -H "Authorization: Bearer $access_token" | jq -r .value)
